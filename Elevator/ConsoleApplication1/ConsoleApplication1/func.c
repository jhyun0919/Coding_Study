		...
		case A:
			{
				if(chosen->direction == UP){
					target = Track_Main[temp->load_level].In_Up_ptr;
					if(target == NULL)
						Track_Main[temp->load_level].In_Up_ptr = chosen;
					else{
					...
					target = Track_Main[temp->load_level].In_Down_ptr;
					if(target == NULL)
						Track_Main[temp->load_level].In_Down_ptr = chosen;
					else{
						while(target->next != NULL){
							target = target->next;
						}
...
void MoveUp_A(){
	...
	if(Track_Main[Elv_A.cur_level].Out_passenger != 0){
		Append_Event_DoorOpen_A();	
	}
	if(Track_Main[Elv_A.cur_level].In_Up_ptr != NULL){
		Append_Event_DoorOpen_A();
	}
	
	if(Track_Main[Elv_A.cur_level].In_Down_ptr != NULL){
		if(anyUpRequest_A() == OFF)
			Append_Event_DoorOpen_A();
	}
...
int anyDownRequest_A(){
	...
	for(check = Elv_A.cur_level; check >= 0; check--){
		isAny = isAny || Track_On_Off(Track_Main[check]);
	}
...
void MoveUp_B(){
	...
	/* append Event_DoorOpen at next tick if EV has arrived */
	if(Track_Main[Elv_B.cur_level].Out_passenger != 0){
		Append_Event_DoorOpen_B();	
	}
	if(Track_Main[Elv_B.cur_level].In_Up_ptr != NULL){
		Append_Event_DoorOpen_B();
	}
	
	if(Track_Main[Elv_B.cur_level].In_Down_ptr != NULL){
		if(anyUpRequest_B() == OFF)
			Append_Event_DoorOpen_B();
	}
