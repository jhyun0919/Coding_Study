...
void     writeReport  (STU_DATA aStudent, FILE *prtFile);
...
/*	Statements */
	ioResult = fread(&aStu, sizeof(STU_DATA), 1, stuFile);
	if (!ioResult)
	   if (!feof(stuFile))
	      {
...
/*	==================== writeReport ====================
	This function writes the student report to a text file.
	   Pre   prtFile is opened as a text output file
	         aStudent is complete
	   Post   Report line written w/ page header if necessary.
*/
void writeReport  (STU_DATA  aStu, 
                   FILE     *prtFile) 
{
/*	Local Definitions */
